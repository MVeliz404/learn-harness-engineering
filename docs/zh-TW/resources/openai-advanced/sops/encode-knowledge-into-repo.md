...
當重要脈絡還散落在 Google Docs、聊天記錄、ticket 或人的腦子裡時，就用這份 SOP。
...
- review 裡經常引用儲存庫裡根本沒寫下來的產品、安全或架構規則。
- 新工作階段總在重複那些本該已經穩定下來的發現工作。

## 執行 SOP

1. 先列出所有不可見知識來源：外部文件、聊天、預設團隊規則、口頭決策。
2. 對每條知識判斷：它屬於架構、產品行為、安全策略、可靠性要求、執行脈絡，還是參考材料？
3. 按類別編碼到對應儲存庫工件：
   - 架構 -> `ARCHITECTURE.md`
...
4. 把模糊表達改寫成執行上真正有用的表達。
5. 刪除或廢棄舊副本，保證儲存庫裡有一個可發現的真相來源。
...
- 決策形成的同一輪工作階段裡，就把儲存庫更新掉。

## 完成定義

- 一個全新 agent 不問人也能找到相關規則。
- 同一個事實不會散落在多個互相打架的檔案裡。
- 新工件放在離它所治理的程式碼或流程最近的地方。
