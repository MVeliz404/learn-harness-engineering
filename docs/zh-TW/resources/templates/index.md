...
收尾檢查清單。每次工作階段結束前過一遍，確保儲存庫處於下一輪可以直接開工的狀態。
...
Quality document 也可以用來驗證 harness 是否可以簡化。Harness 裡的每個元件都編碼了一個假設——"模型做不到這件事"。模型變強後，這些假設就過時了。檢查某個元件是否還有必要：

1. 拍一份 quality document 快照。
2. 移除一個 harness 元件。
3. 跑基準測試。
4. 再拍一份快照。
5. 對比——評級沒降，說明那個元件是多餘的。降了，就恢復。
