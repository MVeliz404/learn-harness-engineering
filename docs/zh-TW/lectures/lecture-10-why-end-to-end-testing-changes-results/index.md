...
Google 的測試金字塔告訴我們：大量單元測試是基礎，但如果你止步於此，就會系統性地漏掉元件互動問題。對於 AI 程式碼代理來說，這個問題更嚴重——agent 傾向於只跑最快的測試然後宣告完成。**只有端到端測試能證明系統級缺陷不存在**。
...
關鍵原則：**執行不變數，不微管實現。** 比如要求「資料在邊界解析」，但不規定用哪個程式庫。錯誤訊息要包含修復指導——不只是說「違規了」，而是告訴 agent 具體怎麼改。
...
- **架構規則必須可執行**——不是寫在文件裡等人來看，而是每次提交自動檢查。
